...
int main() {
    ...
    for (;;) {
        ...
        // rendering body
        {
            ...
            for (int j = 0; j < HEIGHT; j++) line_blur(j * WIDTH, 1, WIDTH);
            for (int i = 0; i < WIDTH; i++) line_blur(i, WIDTH, HEIGHT);

            for (int i = 0; i < WIDTH * HEIGHT; i++)  // cool
                if (rand() % 10 < 10) fire[i] = std::max(fire[i] - 2, 0);
        ...
        for (int j = 0; j < HEIGHT; j++) {  // show the buffer
            for (int i = 0; i < WIDTH; i++)
                std::cout << palette[fire[i + j * WIDTH]];
            std::cout << "\033[49m\n";
        }
